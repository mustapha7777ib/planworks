...
export default function Hero() {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-slate-900">
        ...
        {slides.map((slide, index) => (
          ...
          >
            <Image
              ...
              priority={index === 0} // Prioritize the first image only
              placeholder="blur"      // Shows a blurred version while loading
              sizes="100vw"           // Helps the browser pick the right size
              quality={85}            // Slight compression for better speed
            />
      ...
      {/* INDICATORS */}
      <div className="absolute bottom-12 left-6 flex gap-4 items-center z-20">
        {slides.map((_, index) => (
          <button
            key={index}
